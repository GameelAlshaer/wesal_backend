...
{
    /**
	 * User Login/Sign up with facebook redirect page.
    ...
    {
        return Socialite::driver('facebook')->with(['state' =>'phone='.$request->phone.'&gender='.$request->gender.'&birth_day='.$request->birth_day])->redirect();
    }
    /**
	 * User Login/Sign up with facebook handling.
     *
    ...
    public function handleFacebookCallback(Request $request)
    {
        ...
        if($oldUser){
            ...
            return response()->json(['message' => 'logged in with facebook account successfully ','AccessToken'=>$token], 200);
        }
        else{
            ...
            if ($validator->fails()) {
                ...
                return response()->json(['message' => 'Signed up with facebook account successfully ','AccessToken:'=>$token], 201);
            }
        }
    }
}
